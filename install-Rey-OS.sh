...
if [ -d "zaneyos" ]; then
  ...
  if [ -d ".config/zaneyos-backups" ]; then
    ...
    mv "$HOME"/Rey-OS .config/Rey-OS-backups/"$backupname"
    sleep 1
  else
    echo "Creating the backups folder & moving ZaneyOS to it."
    mkdir -p .config/Rey-OS-backups
    mv "$HOME"/Rey-OS .config/zaneyos-backups/"$backupname"
    sleep 1
  fi
else
...
git clone https://github.com/Rouzihiro/zaneyos.git
cd Rey-OS || exit
